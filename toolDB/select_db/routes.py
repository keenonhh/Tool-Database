...
def search(id):
    rows = db.session.query(Track, Member, Shows, Setlist).filter(Track.id == id).filter(trackbandmember.c.track_id == Track.id).filter(Member.id == trackbandmember.c.member_id).filter(Setlist.track_id == id).filter(Shows.id == Setlist.show_id).all()   
